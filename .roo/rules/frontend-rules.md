...
- **Ask First**: Before introducing any new external package or library to the monorepo, you must ask the user for approval.
- **Justification**: Provide a brief reason why the package is necessary and how it fits the existing stack.
...
- **Default Choice**: `lucide-react` is the preferred and default solution for icons.
- **Usage**: Use the React components provided by `lucide-react`.
- **Consistency**: Maintain the existing stroke-based aesthetic (default 2px stroke) to match the UI.
- **Legacy**: Existing inline SVG icons with active/inactive variants are still supported but new icons should use Lucide.
