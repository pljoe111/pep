...
const DEST_PUBKEY = '11111111111111111111111111111112';
...
async function seedUser(opts?: { balance?: number; lifetime_withdrawn?: number }): Promise<string> {
  ...
  await prisma.ledgerAccount.create({
    data: {
      ...
      lifetime_withdrawn: opts?.lifetime_withdrawn ?? 0,
    },
  });
...
// ─── §4.1 — Happy path: USDT withdrawal confirmed ─────────────────────────────
describe('WithdrawalWorker §4.1 — USDT withdrawal confirmed on-chain', () => {
  it('updates status to confirmed and sets onchain_signature, always sends USDT', async () => {
    const userId = await seedUser();
    // Option B: ledger tx created with 'usdt' currency
    const txId = await seedPendingWithdrawal(userId, '10', 'usdt');
    ...
    // Option B: always sends USDT on-chain
    expect(mockSolana.executeWithdrawal).toHaveBeenCalledWith(DEST_PUBKEY, 10_000_000n, 'usdt');
  });
});

// ─── §4.2 — Idempotency: onchain_signature already set and tx already landed ──
describe('WithdrawalWorker §4.2 — Idempotency: signature pre-set', () => {
  it('marks confirmed and does NOT send a duplicate transfer when signature already exists on-chain', async () => {
    ...
    const txId = await seedPendingWithdrawal(userId, '5', 'usdt', {
...
// ─── §4.3 — Already confirmed: worker returns early ──────────────────────────
describe('WithdrawalWorker §4.3 — Already confirmed: worker returns early', () => {
  it('does nothing when ledger transaction status is already confirmed', async () => {
    const userId = await seedUser();
    const txId = await seedPendingWithdrawal(userId, '5', 'usdt', { status: 'confirmed' });
...
// ─── §4.4 — Permanent failure: balance restored ───────────────────────────────
describe('WithdrawalWorker §4.4 — Permanent failure: balance restored', () => {
  it('restores balance and decrements lifetime_withdrawn on permanent failure', async () => {
    const userId = await seedUser({ balance: 0, lifetime_withdrawn: 10 });
    const txId = await seedPendingWithdrawal(userId, '10', 'usdt');
    ...
    expect(account.balance.toString()).toBe('10'); // balance restored
    expect(account.lifetime_withdrawn.toString()).toBe('0'); // decremented back
...
// ─── §4.5 — from_account_id = null: balance NOT restored ─────────────────────
describe('WithdrawalWorker §4.5 — from_account_id null (fee sweep path)', () => {
  it('does not attempt a LedgerAccount update when from_account_id is null', async () => {
    const txId = await seedPendingWithdrawal(null, '5', 'usdt');
    ...
    expect(mockNotif.send).not.toHaveBeenCalled();
  });
});

// ─── §4.6 — Amount conversion accuracy ───────────────────────────────────────
describe('WithdrawalWorker §4.6 — Amount conversion accuracy', () => {
  ...
  for (const { dbAmount, expectedRaw } of cases) {
    it(`converts DB amount "${dbAmount}" → ${expectedRaw} raw bigint, sends as USDT`, async () => {
      const userId = await seedUser();
      const txId = await seedPendingWithdrawal(userId, dbAmount, 'usdt');

      mockSolana.executeWithdrawal.mockResolvedValue(`wd_sig_${Date.now()}`);

      await withdrawalProcessFn!(makeJob(txId));

      // Option B: always calls with 'usdt'
      expect(mockSolana.executeWithdrawal).toHaveBeenCalledWith(DEST_PUBKEY, expectedRaw, 'usdt');
