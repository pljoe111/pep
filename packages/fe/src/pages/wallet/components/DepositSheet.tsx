...
export function DepositSheet({ isOpen, onClose }: DepositSheetProps) {
  ...
  return (
    <Sheet isOpen={isOpen} onClose={onClose} title="Deposit USDT">
      <div className="space-y-6">
        <div className="text-center space-y-4">
          <p className="text-sm text-text-2">Your deposit address:</p>
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex gap-3">
          ...
          <div className="space-y-1">
            ...
            <p className="text-xs text-amber-800 leading-relaxed">
              Only send USDT (SPL token) on the Solana network. Sending any other token may result
              in permanent loss.
            </p>
          </div>
        </div>
